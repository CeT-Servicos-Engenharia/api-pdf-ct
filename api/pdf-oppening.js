...
// O resto do seu código para pdf-oppening.js permanece exatamente o mesmo...
// (addHeader, addFooter, generatePDF, generateOppeningPDF, etc.)
// ...
...
async function generatePDF(projectData, clientData, engenieerData) {
  ...
  const tableDataEquipmentNextInspections = [
    ["EXTERNA", "INTERNA"],
    [
      `${projectData.inspection.DateNextInspectionPLHExternal}`,
      `${projectData.inspection.DateNextIsnpectionPLHInternal}`
    ]
  ];
  ...
  await drawTableRevisionControl(
    ...
    columnWidthsEquipmentNextInspections,
    rowHeight,
    tableDataEquipmentNextInspections,
    helveticaFont,
    helveticaBoldFont
  );
  ...
  await drawTableRevisionControl(
    ...
    pdfDoc,
    50, // Posição X
    285, // Posição Y (ajuste para evitar sobreposição)
