...
// --- Utilidades de Data (BR) ---
/**
 * Tenta converter qualquer entrada (Timestamp do Firebase, ISO string, número ms, Date)
 * em Date e devolve no formato brasileiro DD/MM/YYYY. Se já estiver em DD/MM/YYYY, mantém.
 */
function formatDateBR(input) {
  if (!input) return "";
  if (typeof input === "string" && /^\d{2}\/\d{2}\/\d{4}$/.test(input)) return input; // já está BR

  // Firebase Timestamp { seconds, nanoseconds } ou { _seconds, _nanoseconds }
  if (typeof input === "object" && input !== null) {
    if (input instanceof Date) return padDateBR(input);
    if (typeof input.seconds === "number") return padDateBR(new Date(input.seconds * 1000));
    if (typeof input._seconds === "number") return padDateBR(new Date(input._seconds * 1000));
  }

  // number (ms) ou string ISO/UTC
  try {
    const d = new Date(input);
    if (!isNaN(d)) return padDateBR(d);
  } catch (_) {
    // ignora
  }

  // fallback: devolve string original se for "legível", senão vazio
  return typeof input === "string" ? input : "";
}

function padDateBR(date) {
...
async function addFooter(pdfDoc, page, data, pageNumber) {
  ...
  // Datas em BR
  const endDateBR = formatDateBR(data?.inspection?.endDate);

  const footerTextStart = `ART:${data.artProjeto}`;
  const footerTextEnd = `C&T.0.1 | ${data.numeroProjeto || " "} | ${endDateBR}`;
  ...
  const textWidthEnd = helveticaFont.widthOfTextAtSize(`C&T.0.1 | ${endDateBR}`, 10);
  const xStart = 50;
...
async function generatePDF(projectData, clientData, engenieerData) {
  ...
  page.drawText(`TERMO DE ABERTURA`, {
    x: xMiddle,
    y: 700,
    size: 24,
    font: helveticaBoldFont,
  ...
  const tableDataRevisionControl = [
    ...
    [`${projectData.fabricante || " "}`, `${projectData.anoFabricacao || " "}`, `${projectData.artProjeto || " "}`],
  ];
  ...
  const tableDataAnotherExample = [
    ...
    [`${projectData.numeroSerie || " "}`, `${projectData.numeroPatrimonio || " "}`, `${projectData.numeroProjeto || " "}`, `${projectData.nomeEquipamento || " "}`],
  ];
  ...
  // Texto com data em BR
  const endDateBR = formatDateBR(projectData?.inspection?.endDate);
  await drawIndentedText(
    page,
    `Na Data ${endDateBR}, foi realizada a inspeção inicial consistindo de exame inicial.\n\nApós análise e testes executados, o equipamento está Aprovado, estando desta forma em plenas condições técnicas de ser operado, desde que mantidos os componentes e acessórios apresentados nesta inspeção.`,
    50,
    510,
    470,
    helveticaFont,
  ...
  const tableDataEquipmentClassification = [
    ...
    [`${projectData.temperaturaProjeto || ""}`, `${(projectData.volume || "") + (projectData.tipoVolume || "")}`, `${projectData.categoriaCaldeira || ""}`],
  ];
  ...
  const tableDataEquipmentValuePression = [
    ...
    [`${(projectData.pressaoMaxima || "") + " " + (projectData.unidadePressaoMaxima || "")}`, `${(projectData.pressaoTeste || "") + " " + (projectData.unidadePressaoMaxima || "")}`],
  ];
  ...
  // Datas futuras em BR
  const nextExternalBR = formatDateBR(projectData?.inspection?.DateNextInspectionPLHExternal);
  const nextInternalBR = formatDateBR(projectData?.inspection?.DateNextIsnpectionPLHInternal); // mantém a key original (com typo) mas formata
  const tableDataEquipmentNextInspections = [["EXTERNA", "INTERNA"], [nextExternalBR, nextInternalBR]];
  let columnWidthsEquipmentNextInspections = [247.64, 247.64];
  await drawTableRevisionControl(page, pdfDoc, 50, 327.5, columnWidthsEquipmentNextInspections, rowHeight, tableDataEquipmentNextInspections, helveticaFont, helveticaBoldFont);

  // Info finais com "DATA DE INSPEÇÃO" em BR
  const tableDataEquipmentInfosFinals = [
    ["LOCAL DE INSTALAÇÃO", "DATA DE INSPEÇÃO"],
    [`${projectData.localInstalacao || ""}`, `${endDateBR}`],
  ];
  let columnWidthsEquipmentInfosFinals = [247.64, 247.64];
  await drawTableRevisionControl(page, pdfDoc, 50, 285, columnWidthsEquipmentInfosFinals, rowHeight, tableDataEquipmentInfosFinals, helveticaFont, helveticaBoldFont);
