...
async function generatePDF(projectData, clientData, engenieerData) {
  ...
  page.drawText(`TERMO DE ABERTURA`, { x: xMiddle, y: 700, size: 24, font: helveticaBoldFont, color: rgb(0, 0, 0) });
  ...
  await drawIndentedText(page, `Na Data ${projectData.inspection.endDate}, foi realizada a inspeção inicial consistindo de exame inicial.\n\nApós análise e testes executados, o equipamento está Aprovado, estando desta forma em plenas condições técnicas de ser operado, desde que mantidos os componentes e acessórios apresentados nesta inspeção.`, 50, 510, 470, helveticaFont, 12, 4, 20);
  ...
  const tableDataEquipmentNextInspections = [["EXTERNA", "INTERNA"], [`${projectData.inspection.DateNextInspectionPLHExternal}`, `${projectData.inspection.DateNextIsnpectionPLHInternal}`]];
